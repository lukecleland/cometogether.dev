...
export function DawWidget({
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden rounded-xl border border-emerald-900/60 bg-zinc-950 text-zinc-200 shadow-xl">
      <div className="flex shrink-0 items-center justify-between border-b border-zinc-800 bg-zinc-900 px-3 py-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <button className={button} onClick={onClose} aria-label="Close DAW">
            ×
          </button>
        </div>
      </div>
      ...
      >
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2 border-b border-zinc-800 p-2">
          <button
            className={`${button} text-emerald-300`}
            onClick={() => void togglePlay()}
            disabled={!active.length || missing || recording || busy}
          >
            {playing ? "Pause" : "Play"}
          </button>
          <button
          ...
          <span className="min-w-16 font-mono text-xs text-emerald-300">
            {time(playhead)}
          </span>
          <button
            className={button}
          ...
          <button
            className={`${button} ${recording ? "text-red-400" : ""}`}
            onClick={() => void startRecording()}
            disabled={busy}
          >
            {recording ? "■ Finish take" : "● Record mic"}
          </button>
          <button
            className={button}
        ...
        {recording && (
          ...
          >
            Recording microphone… Finish take to add it to the shared timeline.
          </p>
        )}
        {busy && (
          ...
          {!active.length ? (
            ...
            <div style={{ width: timelineWidth + 190 }}>
              ...
              {active.map((track, index) => (
                ...
                >
                  <div
                    className="sticky left-0 z-20 flex w-[190px] shrink-0 flex-col gap-1 border-r border-zinc-800 bg-zinc-900 p-2"
                  ...
                  >
                    <input
                      ...
                      value={track.name}
                      maxLength={200}
                      onChange={(e) => publish(track, { name: e.target.value })}
                      className="w-full bg-transparent text-xs outline-none focus:text-emerald-300"
                    />
                      ...
                      onKeyDown={(e) => {
                        if (e.key === "ArrowLeft" || e.key === "ArrowRight") {
                          ...
                          updateNumber(
                            track,
                            "start",
                            track.start + (e.key === "ArrowRight" ? 0.1 : -0.1),
                          );
                        }
                      }}
                    >
            ...
            <button
              ...
              onClick={() => {
                ...
                publish({
                  ...
                  start: Math.min(
                    MAX_DAW_SECONDS -
                      (selectedTrack.trimEnd - selectedTrack.trimStart),
                    selectedTrack.start +
                      selectedTrack.trimEnd -
                      selectedTrack.trimStart,
                  ),
                });
                setSelected(id);
              }}
        ...
        <div className="shrink-0 border-t border-zinc-800 px-3 py-1.5 text-[10px] text-zinc-500">
          Edits shared live · Playback is yours · Drag clips to arrange · Click
          the ruler to seek
        </div>
      </div>
    </div>
